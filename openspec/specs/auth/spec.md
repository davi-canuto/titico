...
The `auth()` helper from Auth.js is used to read the session server-side. No API key auth exists.

#### Scenario: Valid session cookie present
- **WHEN** a request arrives with a valid `next-auth.session-token`
- **THEN** `auth()` returns the session object with `user.id`, `user.email`, `user.name`, `user.image`
