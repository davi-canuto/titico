...
export default function PdfPaymentModal({ open, onClose, productId, isAuthenticated }: Props) {
  ...
  useEffect(() => {
    ...
    intervalRef.current = setInterval(async () => {
      try {
        ...
        if (!res.ok) return
        const data = (await res.json()) as { status: string; downloadUrl?: string | null; downloadPassword?: string | null }
        if (data.status === 'COMPLETED') {
          clearPolling()
          setDeliveryData({ downloadUrl: data.downloadUrl ?? null, downloadPassword: data.downloadPassword ?? null })
          setView('success')
  ...
  async function handleCard() {
    ...
    try {
      ...
      console.error('[PdfPaymentModal/handleCard]', e)
  ...
  async function handleCopyPassword() {
    if (!deliveryData?.downloadPassword) return
    try {
      await navigator.clipboard.writeText(deliveryData.downloadPassword)
  ...
  return (
    ...
    >
      <div className="bg-[#161616] border border-white/10 rounded-xl w-full max-w-sm p-7 relative">
        ...
        {view === 'success' && (
          <>
            <h2 className="text-xl font-black uppercase text-white mb-2">PAGAMENTO CONFIRMADO!</h2>
            {deliveryData?.downloadUrl && deliveryData?.downloadPassword ? (
              <>
                <p className="text-white/50 text-sm mb-5">Seu PDF está pronto. Acesse o link e use a senha para abrir o arquivo.</p>
                <a
                  href={deliveryData.downloadUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="w-full flex items-center justify-center gap-2 bg-[#e3001b] hover:bg-[#b50015] active:bg-[#900010] text-white font-black uppercase tracking-wider text-sm px-5 py-3.5 rounded-lg transition-colors mb-3"
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 border border-white/20 hover:border-white/40 text-white text-sm font-semibold rounded-lg px-4 py-3 transition-colors"
                >
                  {copiedPassword ? (
                    <>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#4ade80" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                      ...
                      <span className="text-[#4ade80]">Senha copiada!</span>
                    </>
                  ) : (
                    <>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                <p className="mt-4 text-xs text-white/30 text-center">Guarde a senha — ela não será exibida novamente</p>
              </>
            ) : (
              <p className="text-white/50 text-sm">
                Pagamento recebido! Porém houve um problema ao recuperar o PDF. Entre em contato com o suporte.
              </p>
            )}
          </>
        )}
