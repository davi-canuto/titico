  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-[#0d0d0d]/90 backdrop-blur border-b border-white/5">
      <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
        <a href="#" className="text-2xl font-black tracking-widest text-white uppercase">
        ...
        <nav className="hidden md:flex items-center gap-8">
          {navLinks.map((l) => (
            <a key={l.href} href={l.href} className="text-sm font-semibold uppercase tracking-wider text-white/70 hover:text-[#e3001b] transition-colors">
              {l.label}
            </a>
          ))}
          <a
      ...
      {open && (
        <div className="md:hidden bg-[#111] border-t border-white/5 px-4 py-4 flex flex-col gap-4">
          {navLinks.map((l) => (
            <a key={l.href} href={l.href} onClick={() => setOpen(false)} className="text-sm font-semibold uppercase tracking-wider text-white/70">
              {l.label}
            </a>
          ))}
          <a href="https://api.whatsapp.com/send/?phone=5512982700714&text=Ol%C3%A1%2C+tudo+bem+%3F%21+Vim+do+site+e+gostaria+de+fazer+um+or%C3%A7amento+%21&type=phone_number&app_absent=0" target="_blank" rel="noopener noreferrer" onClick={() => setOpen(false)} className="bg-[#e3001b] text-white text-sm font-bold uppercase px-4 py-2 rounded text-center">
            Comprar Guia
          </a>
