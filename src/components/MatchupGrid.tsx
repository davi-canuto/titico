...
const filters: Array<Difficulty | 'Todos'> = ['Todos', 'Fácil', 'Médio', 'Difícil']
...
export default function MatchupGrid() {
  ...
  const filtered = filter === 'Todos' ? matchups : matchups.filter((m) => m.difficulty === filter)
        ...
        <div className="flex justify-center gap-3 mb-10 flex-wrap">
          {filters.map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-5 py-2 rounded text-sm font-bold uppercase tracking-wider transition-colors ${
                filter === f
